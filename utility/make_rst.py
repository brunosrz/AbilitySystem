...
# Removed engine-specific imports for standalone use
# import version
...
def force_stderr_color(*args: Any) -> None:
    pass


def force_stdout_color(*args: Any) -> None:
    pass
...
class State:
    def __init__(self) -> None:
        ...
        # Inject basic Godot types to allow standalone document generation
        for basic_type in [
            "Object",
            "Node",
            "Resource",
            ...
            if basic_type == "Object":
                ...
                type_def.enums["SizeFlags"] = EnumDef(
                    "SizeFlags", TypeName("int"), True
                )
            elif basic_type == "Node":
                # Add _process to satisfy [method Node._process]
        ...
        self.script_language_parity_check: ScriptLanguageParityCheck = (
        ...
        if properties is not None:
            for property in properties:
                ...
                if property_name in class_def.properties:
                    print_error(
                        f'{class_name}.xml: Duplicate property "{property_name}".', self
                    )
                    continue
        ...
        if constants is not None:
            for constant in constants:
                ...
                constant_def = ConstantDef(
                    constant_name, value, constant.text, is_bitfield
                )
                ...
                if enum is None:
                    if constant_name in class_def.constants:
                        print_error(
                            f'{class_name}.xml: Duplicate constant "{constant_name}".',
                            self,
                        )
                        continue
        ...
        if annotations is not None:
            for annotation in annotations:
                ...
                annotation_def = AnnotationDef(
                    annotation_name, params, annotation_desc, qualifiers
                )
        ...
        if signals is not None:
            for signal in signals:
                ...
                if signal_name in class_def.signals:
                    print_error(
                        f'{class_name}.xml: Duplicate signal "{signal_name}".', self
                    )
                    continue
    ...
    def sort_classes(self) -> None:
        self.classes = OrderedDict(
            sorted(self.classes.items(), key=lambda t: t[0].lower())
        )
...
class TypeName:
    def __init__(
        self, type_name: str, enum: str | None = None, is_bitfield: bool = False
    ) -> None:
...
class ParameterDef(DefinitionBase):
    def __init__(
        self, name: str, type_name: TypeName, default_value: str | None
    ) -> None:
...
class SignalDef(DefinitionBase):
    def __init__(
        self, name: str, parameters: list[ParameterDef], description: str | None
    ) -> None:
...
class ScriptLanguageParityCheck:
    ...
    def add_hit(
        self, class_name: str, context: DefinitionBase, error: str, state: State
    ) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--lang", "-l", default="en", help="Language to use for section headings."
    )
    parser.add_argument(
        "--color",
        action="store_true",
    ...
    if state.script_language_parity_check.hit_count > 0:
        if not args.verbose:
            ...
            for class_name in state.script_language_parity_check.hit_map.keys():
                ...
                print(
                    f'{Ansi.YELLOW}- {len(class_hits)} hits in class "{class_name}"{Ansi.RESET}'
                )
    ...
    if state.num_errors >= 2:
        ...
        print(
            f"{Ansi.RED}1 error was found in the class reference XML. Please check the messages above.{Ansi.RESET}"
        )

    if state.num_errors == 0:
        print(
            f"{Ansi.GREEN}Class reference RST generation completed successfully.{Ansi.RESET}"
        )
        if not args.dry_run:
...
def make_rst_class(
    class_def: ClassDef, state: State, dry_run: bool, output_dir: str
) -> None:
    ...
    with open(
        ...
        if dry_run
            ...
            output_dir, f"class_{sanitize_class_name(class_name, True)}.rst"
        ),
        "w",
        encoding="utf-8",
        newline="\n",
        ...
        if not writing_translation:  # Skip for translations to reduce diff.
            ...
            if class_def.filepath == "built-in":
                return
            source_xml_path = os.path.relpath(
                class_def.filepath, root_directory
            ).replace("\\", "/")

            # Normalize path to point to src/doc_classes/ even if run from within the engine module.
            if "zyris/modules/" in source_xml_path:
                # Example: zyris/modules/ability_system/doc_classes/AbilitySystem.xml -> src/doc_classes/AbilitySystem.xml
                parts = source_xml_path.split("/")
                if "doc_classes" in parts:
                    source_xml_path = "src/doc_classes/" + parts[-1]

            source_github_url = f"https://github.com/brunosrz/AbilitySystem/tree/{git_branch}/{source_xml_path}"
            generator_github_url = f"https://github.com/brunosrz/AbilitySystem/tree/{git_branch}/utility/make_rst.py"
        ...
        # Brief description
        if (
            class_def.brief_description is not None
            and class_def.brief_description.strip() != ""
        ):
            has_description = True

            f.write(
                f"{format_text_block(class_def.brief_description.strip(), class_def, state)}\n\n"
            )
        ...
        if class_def.description is not None and class_def.description.strip() != "":
            ...
            f.write(
                f"{format_text_block(class_def.description.strip(), class_def, state)}\n\n"
            )
        ...
        if len(class_def.properties) > 0:
            ...
            for property_def in class_def.properties.values():
                ...
                if default is not None and property_def.overrides:
                    ...
                    ml.append(
                        (type_rst, property_def.name, f"{default} (overrides {ref})")
                    )
                else:
        ...
        if len(class_def.signals) > 0:
            ...
            for signal in class_def.signals.values():
                ...
                signal_anchor = (
                    f"class_{sanitize_class_name(class_name)}_signal_{signal.name}"
                )
                ...
                if signal.description is not None and signal.description.strip() != "":
                    f.write(
                        f"{format_text_block(signal.description.strip(), signal, state)}\n\n"
                    )
        ...
        if len(class_def.enums) > 0:
            ...
            for e in class_def.enums.values():
                ...
                for value in e.values.values():
                    ...
                    f.write(
                        f".. _class_{sanitize_class_name(class_name)}_constant_{value.name}:\n\n"
                    )
                    f.write(".. rst-class:: classref-enumeration-constant\n\n")

                    f.write(
                        f"{e.type_name.to_rst(state)} **{value.name}** = ``{value.value}``\n\n"
                    )
                    ...
                    if value.text is not None and value.text.strip() != "":
                        f.write(
                            f"{format_text_block(value.text.strip(), value, state)}"
                        )
        ...
        if len(class_def.constants) > 0:
            ...
            for constant in class_def.constants.values():
                ...
                constant_anchor = (
                    f"class_{sanitize_class_name(class_name)}_constant_{constant.name}"
                )
                ...
                if constant.text is not None and constant.text.strip() != "":
                    f.write(
                        f"{format_text_block(constant.text.strip(), constant, state)}"
                    )
            ...
            for method_list in class_def.annotations.values():  # type: ignore
                for i, m in enumerate(method_list):
                    ...
                    if m.description is not None and m.description.strip() != "":
                        f.write(
                            f"{format_text_block(m.description.strip(), m, state)}\n\n"
                        )
                    else:
                        f.write(".. container:: contribute\n\n\t")
                        f.write(
        ...
        if any(not p.overrides for p in class_def.properties.values()) > 0:
            ...
            for property_def in class_def.properties.values():
                ...
                if (
                    property_def.setter is not None
                    and not property_def.setter.startswith("_")
                ):
                    property_setter = make_setter_signature(
                        class_def, property_def, state
                    )
                    property_setget += f"- {property_setter}\n"

                if (
                    property_def.getter is not None
                    and not property_def.getter.startswith("_")
                ):
                    property_getter = make_getter_signature(
                        class_def, property_def, state
                    )
                ...
                if property_def.text is not None and property_def.text.strip() != "":
                    f.write(
                        f"{format_text_block(property_def.text.strip(), property_def, state)}\n\n"
                    )
                elif (
                    property_def.deprecated is None
                    and property_def.experimental is None
                ):
                    f.write(".. container:: contribute\n\n\t")
                    f.write(
                        translate(
        ...
        if len(class_def.theme_items) > 0:
            ...
            for theme_item_def in class_def.theme_items.values():
                ...
                if (
                    theme_item_def.text is not None
                    and theme_item_def.text.strip() != ""
                ):
                    f.write(
                        f"{format_text_block(theme_item_def.text.strip(), theme_item_def, state)}\n\n"
                    )
                elif (
                    theme_item_def.deprecated is None
                    and theme_item_def.experimental is None
                ):
                    f.write(".. container:: contribute\n\n\t")
                    f.write(
                        translate(
...
def make_type(klass: str, state: State) -> str:
    ...
    if klass.startswith(
        "Dictionary["
    ):  # Typed dictionary, split elements to link contained types.
    ...
    if c in state.classes and e in state.classes[c].enums:
        if is_bitfield:
            if not state.classes[c].enums[e].is_bitfield:
                print_error(
                    f'{state.current_class}.xml: Enum "{t}" is not bitfield.', state
                )
...
def make_method_signature(
    class_def: ClassDef,
    definition: AnnotationDef | MethodDef | SignalDef,
    ref_type: str,
    state: State,
) -> tuple[str, str]:
    ret_type = ""
    ...
    if isinstance(definition, MethodDef) and ref_type != "":
        if ref_type == "operator":
            op_name = definition.name.replace(
                "<", "\\<"
            )  # So operator "<" gets correctly displayed.
...
def make_setter_signature(
    class_def: ClassDef, property_def: PropertyDef, state: State
) -> str:
    ...
    # Otherwise we fake it with the information we have available.
        ...
        setter = MethodDef(
            property_def.setter, TypeName("void"), setter_params, None, None
        )
...
def make_getter_signature(
    ...
    # Otherwise we fake it with the information we have available.
        ...
        getter = MethodDef(
            property_def.getter, property_def.type_name, getter_params, None, None
        )
...
def make_deprecated_experimental(item: DefinitionBase, state: State) -> str:
    ...
    if item.deprecated is not None:
        ...
        if item.deprecated.strip() == "":
            default_message = translate(
                f"This {item.definition_name} may be changed or removed in future versions."
            )
...
def make_footer() -> str:
    ...
    operator_msg = translate(
        "This method describes a valid operator to use with this type as left-hand operand."
    )
    bitfield_msg = translate(
        "This value is an integer composed as a bitmask of the following flags."
    )
...
def make_rst_index(
    grouped_classes: dict[str, list[str]], dry_run: bool, output_dir: str
) -> None:
    with open(
        os.devnull if dry_run else os.path.join(output_dir, "index.rst"),
        "w",
        encoding="utf-8",
        newline="\n",
    ) as f:
...
def format_text_block(
    ...
    code_warning_if_intended_string = (
        "If this is intended, use [code skip-lint]...[/code]."
    )
        ...
        # Tag is a cross-reference or a formatting directive.
            ...
            # Entering codeblocks and inline code tags.
                ...
                if not ignore_code_warnings:
                    ...
                    target_class_name, target_name, *rest = parse_link_target(
                        inside_code_text, state, context_name
                    )
                    ...
                    valid_param_context = isinstance(
                        context, (MethodDef, SignalDef, AnnotationDef)
                    )
            ...
            # Cross-references to items in this or other class documentation pages.
                ...
                if link_target == "":
                    ...
                    if (
                        ...
                        target_class_name, target_name, *rest = parse_link_target(
                            link_target, state, context_name
                        )
                        if len(rest) > 0:
                            print_error(
...
def format_table(
    f: TextIO, data: list[tuple[str | None, ...]], remove_empty_columns: bool = False
) -> None:
    if len(data) == 0:
        return
    ...
    for size in column_sizes:
        ...
        sep += "+" + "-" * (
            size + 2
        )  # Content of each cell is padded by 1 on each side.
    sep += "+\n"
...
def sanitize_operator_name(dirty_name: str, state: State) -> str:
    ...
    if clear_name == "!=":
        ...
        clear_name = "xxx"
        print_error(
            f'Unsupported operator type "{dirty_name}", please add the missing rule.',
            state,
        )
