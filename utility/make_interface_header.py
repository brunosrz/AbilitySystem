...
def generate_gdextension_interface_header(target, source, header_lines=[]):
    ...
    check_allowed_keys(
        data, ["_copyright", "$schema", "format_version", "types", "interface"]
    )
...
""")
        ...
        for type in data["types"]:
            ...
            if "deprecated" in type:
                check_allowed_keys(
                    type["deprecated"], ["since"], ["message", "replace_with"]
                )
                if "replace_with" in type["deprecated"]:
                    type_replacements.append(
                        (type["name"], type["deprecated"]["replace_with"])
                    )
            ...
            if kind == "handle":
                ...
                type["type"] = (
                    "void*" if not type.get("is_const", False) else "const void*"
                )
                write_simple_type(file, type)
                handles.append(type["name"])
            elif kind == "alias":
                check_allowed_keys(
                    type, ["name", "kind", "type"], ["description", "deprecated"]
                )
                write_simple_type(file, type)
            elif kind == "enum":
                check_allowed_keys(
                    type,
                    ["name", "kind", "values"],
                    ["is_bitfield", "description", "deprecated"],
                )
                ...
                check_allowed_keys(
                    type,
                    ["name", "kind", "arguments"],
                    ["return_value", "description", "deprecated"],
                )
                write_function_type(file, type)
            elif kind == "struct":
                check_allowed_keys(
                    type, ["name", "kind", "members"], ["description", "deprecated"]
                )
        ...
        for type_name, replace_with in type_replacements:
            if replace_with not in valid_data_types:
                raise Exception(
                    f"Unknown type '{replace_with}' used as replacement for '{type_name}'"
                )
        ...
        for interface in data["interface"]:
            ...
            if "deprecated" in interface:
                check_allowed_keys(
                    interface["deprecated"], ["since"], ["message", "replace_with"]
                )
                if "replace_with" in interface["deprecated"]:
                    interface_replacements.append(
                        (interface["name"], interface["deprecated"]["replace_with"])
                    )
...
def write_simple_type(file, type):
    file.write(
        f"typedef {format_type_and_name(type['type'], type['name'])};{make_deprecated_comment_for_type(type)}\n"
    )
...
def write_function_type(file, fn):
    ...
    file.write(
        f"typedef {format_type_and_name(return_type, name_and_args)};{make_deprecated_comment_for_type(fn)}\n"
    )
...
def write_interface(file, interface):
    ...
    if "arguments" in interface:
        ...
        for arg in interface["arguments"]:
            if "description" not in arg:
                raise Exception(
                    f"Interface function {interface['name']} is missing docs for {arg['name']} argument"
                )
    ...
    if "return_value" in interface:
        if "description" not in interface["return_value"]:
            raise Exception(
                f"Interface function {interface['name']} is missing docs for return value"
            )
    ...
    if "deprecated" in fn:
        del fn["deprecated"]
    fn["name"] = "GDExtensionInterface" + "".join(
        word.capitalize() for word in interface["name"].split("_")
    )
