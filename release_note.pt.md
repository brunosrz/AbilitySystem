...
## 🎮 6. Novo Projeto Demo Funcional - Suporte a Modo Duplo

Foi adicionado um projeto demo simples, mas funcional com suporte para **Singleplayer e Multiplayer**:

### **Modo Singleplayer**:

- **Jogabilidade Solo**: Progressão completa e sistema de habilidades totalmente funcional
- **Implementação de Referência**: Demonstra movimento de personagem, ativação de habilidades e gerenciamento de estado
- **Sistema de IA Completo**: Behavior trees do LimboAI para inimigos e dummies operando num ambiente unificado
- **Progressão**: Árvores de habilidades, gerenciamento de inventário e aprimoramentos de itens

### **Modo Multiplayer** (Arquitetura Cliente-Servidor):

- **Comunicação de Rede**: Sincronização completa de jogadores com autoridade do servidor
- **Servidor Dedicado**: Servidor headless dedicado para sessões de jogo
- **Registro de Jogadores**: Autenticação de usuários e gerenciamento de contas
- **Chat em Tempo Real**: Mensagens em jogo entre jogadores conectados
- **Comandos de Admin**: Painel de terminal com comandos do servidor
  - `players` - Lista todos os jogadores conectados
  - `kick` - Remove jogadores da sessão
  - `broadcast` - Envia mensagens a todos os clientes
  - E mais...
- **Sistema RBAC**: Controle de acesso baseado em função para privilégios de admin
- **Multiplataforma**: Compilação para Windows, Linux, macOS e Android

### **Recursos da Demo**:

- **Menu Principal**: Seleção de modo (Singleplayer/Multiplayer)
- **Configuração Multiplayer**: Configurações de IP/Porta para conexão com servidor
- **Sistema HUD**: Barras de saúde, cooldowns de habilidades e indicadores de status
- **Menu de Pausa**: Pausa em jogo com configurações
- **Renderização Personalizada**: Nomes de jogadores renderizados via `_draw()` (nunca vira com o personagem)
