extends Node
## ASComponent wrapper for dummies - integrates Ability System with dummy combat
## Manages ability activation, effects, and ability-based damage scaling

class_name ASComponentDummy
...
func _ready() -> void:
	...
	print("[ASComponentDummy] Initialized for %s" % owner_node.name)
...
func apply_container(new_container: ASAbilityContainer) -> void:
	...
	if container:
		print("[ASComponentDummy] Container applied with %d abilities" % container.get_abilities().size())
...
func _setup_attributes() -> void:
	if not attribute_set:
		...
		attribute_set.set_attribute_base_value_by_tag(&"attack_power", 2.0)
