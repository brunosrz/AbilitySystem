...
## 🎮 6. New Functional Demo - Dual Mode Support

A new, simple but functional demo project has been added with support for **both Singleplayer and Multiplayer modes**:

### **Singleplayer Mode**:

- **Solo Gameplay**: Full character progression and ability system gameplay
- **Reference Implementation**: Demonstrates character movement, ability activation, and state management
- **Complete AI System**: LimboAI behavior trees for enemies and dummies operating in unified environment
- **Progression**: Skill trees, inventory management, and item upgrades

### **Multiplayer Mode** (Client-Server Architecture):

- **Network Communication**: Full player synchronization with server authority
- **Dynamic Server**: Dedicated headless server for game sessions
- **Player Registration**: User authentication and account management system
- **Real-Time Chat**: In-game messaging between connected players
- **Admin Commands**: Terminal-based admin panel with server commands
  - `players` - List all connected players
  - `kick` - Remove players from session
  - `broadcast` - Send messages to all clients
  - And more...
- **RBAC System**: Role-based access control for admin privileges
- **Cross-Platform**: Build for Windows, Linux, macOS, and Android

### **Demo Features**:

- **Main Menu**: Mode selection (Singleplayer/Multiplayer)
- **Multiplayer Configuration**: IP/Port settings for server connection
- **HUD System**: Health bars, ability cooldowns, and status indicators
- **Pause Menu**: In-game pause with settings
- **Custom Rendering**: Player names rendered via `_draw()` (never flips with character)
